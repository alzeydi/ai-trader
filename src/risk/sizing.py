...
_RISK_BY_TYPE: dict[str, str] = {
    ...
    "F": "risk_per_trade_type_f",
    "G": "risk_per_trade_type_g",
}
...
def size_position(
    ...
    *,
    ...
    # Types E and G: SL is structural (bounce_low / bounce_high) and can sit
    # 4–8 % away from entry. Deriving TP from sl_distance × R:R ratio would
    # push it 8–16 % — unreachable on a typical 4h bounce/breakdown.
    # Instead, anchor TP to the 4h ATR (stored in atr_14 for E and G), which
    # reflects actual 4h volatility regardless of how far the structural
    # reference happens to sit. SL stays structural; only TP changes.
    if (
        candidate.entry_type in ("E", "G")
        and candidate.sl_price_hint is not None
        and atr > 0
    ):
