...
**Cooldown-кэш (важно):** в `src/llm/veto.py:32` константа `LLM_VETO_COOLDOWN_SEC = 600`. Если идентичный кандидат `(symbol, side, entry_type)` повторяется в течение 10 минут — Claude **не вызывается**, переиспользуется предыдущий ответ. Кэш in-process (теряется при перезапуске). Экономит деньги, но: одинаковые кандидаты автоматически получают одинаковый вердикт без нового анализа.
